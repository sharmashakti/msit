...
namespace StatlerWaldorfCorp.TeamService
{
    ...
    public class MembersController : Controller
    {
        ...
        [HttpGet]
        [Route("/[controller]/{memberId}/team")]
        public async virtual Task<IActionResult> GetTeam(Guid memberId)
        {
            //return this.Ok("Get the members");

            var teams = repository.GetTeams();

            var team = teams.FirstOrDefault(t => t.Members.Any(m => m.ID == memberId));

            if (team == null)
            {
                return this.Ok("teamId is not matching for member id " + memberId.ToString());
            }
            else
            {
				return this.Ok(team);               
